package edu.depaul.cdm.madl.tools.ui.watchdog;
...
public class WatchdogThread extends Thread {
  ...
  @Override
  public void run() {
    MadlCore.logInformation("WatchdogThread running");
    ...
    while (!stop) {
      ...
      if (blockedJobstStartTimeMilliseconds > 0 && blockedJobsEvent == null
        ...
        blockedJobsEvent = new Event(blockedJobstStartTimeMilliseconds,
            blockedTaskInfo.getTaskName(), blockedTaskInfo.getReason());
      ...
      try {
        ...
        MadlCore.logInformation("Thread sleep interrupted", e);
      }
    }
  }
  ...
  private void eventCompleted(Event event, long nextEventStartTime) {
    if (event != null && !event.isEventExcluded()) {
      ...
      try {
        MadlCore.logInformation(event.toLogMessage(instrumentation));
      } catch (RuntimeException e) {
        //ss
        //instrumentation.record(e);
        MadlCore.logInformation("Failed to log Watchdog Event", e);
      } finally {
        instrumentation.log();
      }
    }
