...
import edu.depaul.cdm.madl.tools.ui.actions.GenerateMadldocAction;
import edu.depaul.cdm.madl.tools.ui.actions.GenerateJavascriptAction;
...
//            helpToolBar.add(searchComboItem);
  ...
  protected void makeActions(final IWorkbenchWindow window) {
    ...
    madlRunAction = new MadlRunAction(window);

    deployOptimizedAction = new GenerateJavascriptAction(window);

    generateMadldocAction = new GenerateMadldocAction(window);
...
//    register(saveAsAction);
    ...
    aboutAction = new AboutMadlAction(window);
...
//    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(MadlUI.ID_APPS_VIEW);
//    menu.add(new AccessibleShowViewAction(window, viewDesc, false));
    ...
    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(MadlUI.ID_CALL_HIERARCHY);
    menu.add(new AccessibleShowViewAction(window, viewDesc, false));

    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(DebuggerView.ID);
    menu.add(new AccessibleShowViewAction(window, viewDesc, false));

    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(MadlUI.ID_FILE_EXPLORER);
    menu.add(new AccessibleShowViewAction(window, viewDesc, false));
    ...
    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(
        "edu.depaul.cdm.madl.dev.util.ast.ASTExplorer"); //$NON-NLS-1$
    if (viewDesc != null) {
      menu.add(new AccessibleShowViewAction(window, viewDesc, false));
    }
    ...
    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(
        "edu.depaul.cdm.madl.dev.util.analysis.AnalysisView"); //$NON-NLS-1$
    if (viewDesc != null) {
      menu.add(new AccessibleShowViewAction(window, viewDesc, false));
    }
    ...
    viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(MadlUI.ID_PROBLEMS);
    menu.add(new AccessibleShowViewAction(window, viewDesc, false));

    if (MadlCoreDebug.ENABLE_TESTS_VIEW) {
      viewDesc = WorkbenchPlugin.getDefault().getViewRegistry().find(MadlUI.ID_MADLUNIT_VIEW);
      menu.add(new AccessibleShowViewAction(window, viewDesc, false));
    }
  }
...
//    menu.add(actionFactory.getPropertiesItem());
  ...
  private MenuManager createToolsMenu() {
    ...
    menu.add(generateMadldocAction);
...
//            	((RetargetActionWithDefault)buildProjectAction).setDefaultHandler(buildHandler);
  ...
  private void updateContextSensitiveMenuItems(IMenuManager manager) {
    try {
      SubContributionItem item = (SubContributionItem) manager.findUsingPath(MadlEditorActionDefinitionIds.OPEN_EDITOR);
