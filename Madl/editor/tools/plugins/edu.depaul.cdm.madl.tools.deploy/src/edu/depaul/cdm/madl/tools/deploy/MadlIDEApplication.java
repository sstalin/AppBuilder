...
// import com.google.madl.tools.debug.core.util.ResourceServerManager;
...
public class MadlIDEApplication implements IApplication {
  ...
  private static class ConfirmMultipleEditorsDialog extends MessageDialog {
    ...
    public ConfirmMultipleEditorsDialog(Shell parentShell) {
      super(parentShell, WorkbenchMessages.MadlIDEApplication_already_running_dialog_title, null,
          WorkbenchMessages.MadlIDEApplication_already_running_dialog_msg, MessageDialog.QUESTION,
          new String[] {
              WorkbenchMessages.MadlIDEApplication_already_running_dialog_run_button,
              WorkbenchMessages.MadlIDEApplication_already_running_dialog_cancel_button}, 1);
    }
...
//ss no resource server implemented yet
    /*
     * try { new ServerSocket(ResourceServerManager.PREFERRED_PORT).close(); } catch (IOException e)
     * { return true; }
     */
      ...
      try {
        if (Util.isMac()) {
          workspaceUrl = new URL("file", null, System.getProperty("user.home") //$NON-NLS-1$ //$NON-NLS-2$
              + "/Library/Application Support/MadlEditor"); //$NON-NLS-1$
        } else if (Util.isWindows()) {
          File workspaceDir = new File(userHomeDir, "MadlEditor"); //$NON-NLS-1$
