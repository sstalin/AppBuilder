...
//import edu.depaul.cdm.madl.engine.ast.FormalParameterList;
//import edu.depaul.cdm.madl.engine.ast.FunctionDeclaration;
//import edu.depaul.cdm.madl.engine.ast.FunctionExpression;
//import edu.depaul.cdm.madl.engine.ast.FunctionTypeAlias;
//import edu.depaul.cdm.madl.engine.ast.MethodDeclaration;

import edu.depaul.cdm.madl.engine.ast.SimpleIdentifier;

//import edu.depaul.cdm.madl.engine.ast.TopLevelVariableDeclaration;
//import edu.depaul.cdm.madl.engine.ast.TypeName;
//import edu.depaul.cdm.madl.engine.ast.VariableDeclaration;
//import edu.depaul.cdm.madl.tools.ui.MadlElementImageDescriptor;
...
/*   // private static final ImageDescriptorRegistry registry = MadlToolsPlugin.getImageDescriptorRegistry();

    private static ImageDescriptor getBaseImageDescriptor(ASTNode node, boolean isPrivate) {
      if (node instanceof ClassDeclaration) {
        return isPrivate ? MadlPluginImages.DESC_MADL_CLASS_PRIVATE
            : MadlPluginImages.DESC_MADL_CLASS_PUBLIC;
      }
      if (node instanceof FunctionTypeAlias) {
        return isPrivate ? MadlPluginImages.DESC_MADL_FUNCTIONTYPE_PRIVATE
            : MadlPluginImages.DESC_MADL_FUNCTIONTYPE_PUBLIC;
      }
      if (node instanceof VariableDeclaration) {
        return isPrivate ? MadlPluginImages.DESC_MADL_FIELD_PRIVATE
            : MadlPluginImages.DESC_MADL_FIELD_PUBLIC;
      }
      if (node instanceof FunctionDeclaration || node instanceof ConstructorDeclaration
          || node instanceof MethodDeclaration) {
        return isPrivate ? MadlPluginImages.DESC_MADL_METHOD_PRIVATE
            : MadlPluginImages.DESC_MADL_METHOD_PUBLIC;
      }
      return null;
    }
    ...
    private static ImageDescriptor getImageDescriptor(ASTNode node, boolean isPrivate) {
      ...
      if (node instanceof ClassDeclaration) {
        ...
        if (classDeclaration.getAbstractKeyword() != null) {
          flags |= MadlElementImageDescriptor.ABSTRACT;
        }
      }
      // ConstructorDeclaration
      if (node instanceof ConstructorDeclaration) {
        flags |= MadlElementImageDescriptor.CONSTRUCTOR;
      }
      // MethodDeclaration
      if (node instanceof MethodDeclaration) {
        ...
        if (method.isStatic()) {
          flags |= MadlElementImageDescriptor.STATIC;
        }
        if (method.isGetter()) {
          flags |= MadlElementImageDescriptor.GETTER;
        }
        if (method.isSetter()) {
          flags |= MadlElementImageDescriptor.SETTER;
        }
      }
      // done
      return new MadlElementImageDescriptor(base, flags, SIZE);
    }
...
/*    ClassDeclaration enclosingClass = node.getAncestor(ClassDeclaration.class);
   ...
   */
  public static ITreeContentProvider newTreeContentProvider(MadlEditor editor) {
