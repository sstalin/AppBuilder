...
//import edu.depaul.cdm.madl.tools.ui.actions.InstrumentedAction;
...
//import edu.depaul.cdm.madl.tools.ui.internal.viewsupport.ColoredViewersManager;

/**
 * {@link IContentOutlinePage} for {@link MadlEditor}.
 */
public class MadlOutlinePage extends Page implements IContentOutlinePage {
  ...
  /*private class CollapseAllAction extends InstrumentedAction {
    CollapseAllAction() {
      ...
      MadlPluginImages.setLocalImageDescriptors(this, "collapseall.gif"); //$NON-NLS-1$
      PlatformUI.getWorkbench().getHelpSystem().setHelp(
          this,
          MadlHelpContextIds.COLLAPSE_ALL_ACTION);
    }
  ...
  private class MadlOutlineViewer extends TreeViewer {
    public MadlOutlineViewer(final Tree tree) {
      super(tree);
  ...
  private class ExpandAllAction extends InstrumentedAction {
    ExpandAllAction() {
      ...
      MadlPluginImages.setLocalImageDescriptors(this, "expandall.gif"); //$NON-NLS-1$
      PlatformUI.getWorkbench().getHelpSystem().setHelp(this, MadlHelpContextIds.EXPAND_ALL_ACTION);
    }
  ...
  private class LexicalSortingAction extends InstrumentedAction {
    public LexicalSortingAction() {
      PlatformUI.getWorkbench().getHelpSystem().setHelp(
          this,
          MadlHelpContextIds.LEXICAL_SORTING_OUTLINE_ACTION);
      setText(MadlEditorMessages.JavaOutlinePage_Sort_label);
      MadlPluginImages.setLocalImageDescriptors(this, "alphab_sort_co.gif"); //$NON-NLS-1$
      setToolTipText(MadlEditorMessages.JavaOutlinePage_Sort_tooltip);
      setDescription(MadlEditorMessages.JavaOutlinePage_Sort_description);
      // restore selection
      boolean checked = MadlToolsPlugin.getDefault().getPreferenceStore().getBoolean(
    ...
    private void valueChanged(final boolean on, boolean store) {
      ...
      if (store) {
        MadlToolsPlugin.getDefault().getPreferenceStore().setValue(
...
*/
 ...
 // private final String contextMenuID;
  private MadlEditor editor;
  //private MadlOutlineViewer viewer;
  ...
  public MadlOutlinePage(String contextMenuID, MadlEditor editor) {
  ...
  private void contextMenuAboutToShow(IMenuManager menu) {
    MadlToolsPlugin.createStandardGroups(menu);
  ...
  public void createControl(Composite parent) {
    ...
    // create "viewer"
    viewer = new MadlOutlineViewer(tree);
    ...
    actionGroups = new CompositeActionGroup(new ActionGroup[] {
        ...
        new MadlSearchActionGroup(site)});
    // configure actions
    {
