...
import edu.depaul.cdm.madl.tools.ui.IWorkingCopyManager;
...
//import edu.depaul.cdm.madl.tools.ui.actions.GenerateActionGroup;
//import edu.depaul.cdm.madl.tools.ui.actions.RefactorActionGroup;

//import edu.depaul.cdm.madl.tools.ui.internal.actions.NewSelectionConverter;
...
//import edu.depaul.cdm.madl.tools.ui.internal.text.comment.CommentFormattingContext;

import edu.depaul.cdm.madl.tools.ui.internal.text.madl.MadlReconcilingStrategy;
import edu.depaul.cdm.madl.tools.ui.internal.text.madl.IMadlReconcilingListener;

import edu.depaul.cdm.madl.tools.ui.internal.text.madl.IMadlReconcilingListener_OLD;
//import edu.depaul.cdm.madl.tools.ui.internal.text.functions.ContentAssistPreference;
//import edu.depaul.cdm.madl.tools.ui.internal.text.functions.MadlHeuristicScanner;
import edu.depaul.cdm.madl.tools.ui.internal.text.functions.SmartBackspaceManager;
//import edu.depaul.cdm.madl.tools.ui.internal.text.functions.Symbols;

import edu.depaul.cdm.madl.tools.ui.internal.util.MadlModelUtil;
 ...
 * Madl code editor.
 */
public class CompilationUnitEditor extends MadlEditor implements IMadlReconcilingListener_OLD {
	
  class AdaptedSourceViewer extends MadlSourceViewer {
...
/*    @Override
    public IFormattingContext createFormattingContext() {
      ...
      MadlElement inputMadlElement = getInputMadlElement();
      MadlProject madlProject = inputMadlElement != null ? inputMadlElement.getMadlProject() : null;
      if (madlProject == null) {
        preferences = new HashMap<String, String>(MadlCore.getOptions());
      } else {
        preferences = new HashMap<String, String>(madlProject.getOptions(true));
      }
...
/*  private class BracketInserter implements VerifyKeyListener, ILinkedModeListener {
    ...
    public void verifyKey(VerifyEvent event) {
      ...
      try {
        ...
        MadlHeuristicScanner scanner = new MadlHeuristicScanner(document);
...
//          break;
  ...
  private class RememberedOffset {
    ...
    private int fColumn;
    /** Remembered Madl element for the given offset */
    ...
    /**
     * Return Madl element recomputed from stored visual properties.
     * 
     * @return Madl element
     */
...
/*    public int getOffset() {
      ...
      Object ElementnewElement = getElement();
    ...
    //ss
    /*
    public int getRememberedOffset(Object ElementnewElement) {

      if (newElement instanceof MadlElement) {
        ...
        try {
          ...
          // should not happen
          MadlToolsPlugin.log(e.getStatus());
          return -1;
        }
      }
    ...
    */
     ...
     */
    //ss
    /*public int getRevealOffset(Object Elementelement, int offset) {
      ...
      if (containsOffset(element, offset)) {
        if (offset > 0) {
          MadlElement alternateElement = (MadlElement) getElementAt(offset, false);
          if (((MadlElement) element).getHandleIdentifier().equals(
    ...
    /**
     * Does the given Madl element contain the given offset?
     * 
     * @param element Madl element
     * @param offset Offset
     * @return <code>true</code> iff the Madl element contains the offset
     */
    ...
    /**
     * Returns the updated Madl element for the old Madl element.
     * 
     * @param element Old Madl element
     * @return Updated Madl element
     */
    private Object /*Element*/findElement(Object /*Element*/element) {
      ...
      if (element instanceof MadlElement) {
        ...
        IWorkingCopyManager manager = MadlToolsPlugin.getDefault().getWorkingCopyManager();
        CompilationUnit unit = manager.getWorkingCopy(getEditorInput());

        if (unit != null) {
          try {
            MadlModelUtil.reconcile(unit);
            MadlElement[] findings = unit.findElements((MadlElement) element);
            if (findings != null && findings.length > 0) {
              return findings[0];
            }

          } catch (MadlModelException x) {
            MadlToolsPlugin.log(x.getStatus());
     ...
     * @param element the Madl element, may be <code>null</code>
    ...
    private int getElementLine(IDocument document, Object /*Element*/element)
        throws BadLocationException, MadlModelException {
    ...
    /**
     * Returns the length of the given Madl element.
     * 
     * @param element Madl element
     * @return Length of the given Madl element
     */
    ...
    /**
     * Returns the offset of the given Madl element.
     * 
     * @param element Madl element
     * @return Offset of the given Madl element
     */
...
//          MadlElement newStartElement = (MadlElement) fStartOffset.getElement();
...
//          }
//
//          MadlElement newEndElement = (MadlElement) fEndOffset.getElement();
...
//          element = (MadlElement) fEndOffset.getElement();
//          if (element == null) {
//            element = (MadlElement) fStartOffset.getElement();
//          }
//          if (element != null) {
...
//    }
  ...
  private static final boolean CODE_ASSIST_DEBUG = "true".equalsIgnoreCase(Platform.getDebugOption("edu.depaul.cdm.madl.tools.ui/debug/ResultCollector")); //$NON-NLS-1$//$NON-NLS-2$
   ...
   */
  //private MadlEditorErrorTickUpdater fJavaEditorErrorTickUpdater;
  ...
  private MadlReconcilingStrategy madlReconcilingStrategy;
  ...
  public CompilationUnitEditor() {
    super();
    setDocumentProvider(MadlToolsPlugin.getDefault().getCompilationUnitDocumentProvider());
    setEditorContextMenuId("#MadlEditorContext"); //$NON-NLS-1$
    setRulerContextMenuId("#MadlRulerContext"); //$NON-NLS-1$
   // setOutlinerContextMenuId("#JavaScriptOutlinerContext"); //$NON-NLS-1$
    scheduleReconcileAfterBuild();
    // don't set help contextId, we install our own help context
    //fSavePolicy = null;

   // fJavaEditorErrorTickUpdater = new MadlEditorErrorTickUpdater(this);
    MadlX.todo("actions");
    // fCorrectionCommands = null;
  }

  /*
   * @see edu.depaul.cdm.madl.tools.ui.functions.java.IJavaReconcilingListener# aboutToBeReconciled()
   */
  @Override
  public void aboutToBeReconciled() {

    // Notify AST provider
   // MadlToolsPlugin.getDefault().getASTProvider().aboutToBeReconciled(getInputMadlElement());

    // Notify listeners
    Object[] listeners = fReconcilingListeners_OLD.getListeners();
    for (int i = 0, length = listeners.length; i < length; ++i) {
      ((IMadlReconcilingListener_OLD) listeners[i]).aboutToBeReconciled();
    }
  }
   ...
   */
  public void addReconcileListener(IMadlReconcilingListener listener) {
  ...
  public void applyResolvedUnit(edu.depaul.cdm.madl.engine.ast.CompilationUnit unit) {
    ...
    if (unit != null) {
      // notify listeners
      {
        ...
        for (int i = 0, length = listeners.length; i < length; ++i) {
          ((IMadlReconcilingListener) listeners[i]).reconciled(unit);
        }
      }
    }
  }
    ...
    if (p.isDeleted(getEditorInput())) {
      ...
      if (isSaveAsAllowed()) {
        ...
        MessageDialog.openError(
            shell,
            MadlEditorMessages.CompilationUnitEditor_error_saving_title1,
            MadlEditorMessages.CompilationUnitEditor_error_saving_message1);
      }
...
//    addAction(menu, "OrganizeImports");
  ...
  /*
   * @see edu.depaul.cdm.madl.tools.ui.editor.JavaEditor#getAdapter(java.lang.Class)
   */
  ...
  public Object getAdapter(Class required) {
    if (SmartBackspaceManager.class.equals(required)) {
      if (getSourceViewer() instanceof MadlSourceViewer) {
        return ((MadlSourceViewer) getSourceViewer()).getBackspaceManager();
      }
    }
  ...
  public MadlReconcilingStrategy getMadlReconcilingStrategy() {
    return madlReconcilingStrategy;
  }
  ...
  @Override
  public void reconciled(MadlUnit ast, boolean forced, IProgressMonitor progressMonitor) {
   // AutoSaveHelper.reconciled(getEditorInput(), getSourceViewer(), getTextSelectionRange());
    ...
    MadlToolsPlugin madlPlugin = MadlToolsPlugin.getDefault();
    if (madlPlugin == null) {
      return;
    }
    ...
    //madlPlugin.getASTProvider().reconciled(ast, getInputMadlElement(), progressMonitor);

    // Notify listeners
    Object[] listeners = fReconcilingListeners_OLD.getListeners();
    for (int i = 0, length = listeners.length; i < length; ++i) {
      ((IMadlReconcilingListener_OLD) listeners[i]).reconciled(ast, forced, progressMonitor);
    }
   ...
   */
  public void removeReconcileListener(IMadlReconcilingListener listener) {
    synchronized (fReconcilingListeners) {
  ...
  @Override
  public void setMadlReconcilingStrategy(MadlReconcilingStrategy madlReconcilingStrategy) {
    this.madlReconcilingStrategy = madlReconcilingStrategy;
  }
...
/*  @Override
  protected void createActions() {
    ...
    PlatformUI.getWorkbench().getHelpSystem().setHelp(
        action,
        MadlHelpContextIds.CONTENT_ASSIST_ACTION);
    ...
    PlatformUI.getWorkbench().getHelpSystem().setHelp(
        action,
        MadlHelpContextIds.PARAMETER_HINTS_ACTION);

    action = new TextOperationAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "Comment.", this, ITextOperationTarget.PREFIX); //$NON-NLS-1$
    action.setActionDefinitionId(MadlEditorActionDefinitionIds.COMMENT);
    setAction("Comment", action); //$NON-NLS-1$
    markAsStateDependentAction("Comment", true); //$NON-NLS-1$
    PlatformUI.getWorkbench().getHelpSystem().setHelp(action, MadlHelpContextIds.COMMENT_ACTION);

    action = new TextOperationAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "Uncomment.", this, ITextOperationTarget.STRIP_PREFIX); //$NON-NLS-1$
    action.setActionDefinitionId(MadlEditorActionDefinitionIds.UNCOMMENT);
    setAction("Uncomment", action); //$NON-NLS-1$
    markAsStateDependentAction("Uncomment", true); //$NON-NLS-1$
    PlatformUI.getWorkbench().getHelpSystem().setHelp(action, MadlHelpContextIds.UNCOMMENT_ACTION);

    action = new ToggleCommentAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "ToggleComment.", this); //$NON-NLS-1$
    action.setActionDefinitionId(MadlEditorActionDefinitionIds.TOGGLE_COMMENT);
    setAction("ToggleComment", action); //$NON-NLS-1$
    markAsStateDependentAction("ToggleComment", true); //$NON-NLS-1$
    PlatformUI.getWorkbench().getHelpSystem().setHelp(
        action,
        MadlHelpContextIds.TOGGLE_COMMENT_ACTION);
    configureToggleCommentAction();

    action = new TextOperationAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "Format.", this, ISourceViewer.FORMAT); //$NON-NLS-1$
    action.setActionDefinitionId(MadlEditorActionDefinitionIds.FORMAT);
    setAction("Format", action); //$NON-NLS-1$
    markAsStateDependentAction("Format", true); //$NON-NLS-1$
    markAsSelectionDependentAction("Format", true); //$NON-NLS-1$
    PlatformUI.getWorkbench().getHelpSystem().setHelp(action, MadlHelpContextIds.FORMAT_ACTION);

//    action = new OrganizeImportsAction(this);
//    action.setActionDefinitionId(MadlEditorActionDefinitionIds.ORGANIZE_IMPORTS);
//    setAction("OrganizeImports", action);
    ...
    //     MadlHelpContextIds.ADD_BLOCK_COMMENT_ACTION);
    ...
    //     PlatformUI.getWorkbench().getHelpSystem().setHelp(action,
    //     MadlHelpContextIds.REMOVE_BLOCK_COMMENT_ACTION);

    //     action = new
    //     IndentAction(MadlEditorMessages.getBundleForConstructedKeys(),
    ...
    //     PlatformUI.getWorkbench().getHelpSystem().setHelp(action,
    //     MadlHelpContextIds.INDENT_ACTION);
    ...
    MadlMoveLinesAction[] moveLinesActions = MadlMoveLinesAction.createMoveCopyActionSet(
        MadlEditorMessages.getBundleForConstructedKeys(),
        this);
...
//             ITextEditorActionConstants.GROUP_EDIT)
  ...
  /*
   * @see edu.depaul.cdm.madl.tools.ui.editor.MadlEditor#createMadlSourceViewer(org.eclipse
  ...
  /*
   * @see edu.depaul.cdm.madl.tools.ui.editor.JavaEditor#createNavigationActions()
   */
  @Override
  ...
  /*  if (fJavaEditorErrorTickUpdater != null) {
      ...
      if (input instanceof FileStoreEditorInput) {
        ...
        if (MadlCoreDebug.ENABLE_NEW_ANALYSIS) {
          ...
          fJavaEditorErrorTickUpdater.updateEditorImage(getInputMadlElement());
        }
  ...
  /*
   * @see MadlEditor#getCorrespondingElement(MadlElement)
   */
  @Override
  protected MadlElement getCorrespondingElement(MadlElement element) {
...
//ss
  /*  if (MadlCoreDebug.ENABLE_NEW_ANALYSIS) {
    ...
    MadlElement element = getInputMadlElement();
    if (element instanceof MadlLibrary) {
      try {
        element = ((MadlLibrary) element).getDefiningCompilationUnit();
...
//ss
  ...
  protected void openSaveErrorDialog(String title, String message, CoreException exception) {
    ...
    if (MadlUI.ID_PLUGIN.equals(status.getPlugin())
        && status.getCode() == MadlStatusConstants.EDITOR_POST_SAVE_NOTIFICATION) {
      ...
      ErrorDialog dialog = new ErrorDialog(getSite().getShell(), title, message, status, mask) {
        ...
        protected Control createDialogArea(Composite parent) {
          ...
          link.setText(MadlEditorMessages.CompilationUnitEditor_error_saving_saveParticipant);
          link.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
              PreferencesUtil.createPreferenceDialogOn(
                  getShell(),
                  "edu.depaul.cdm.madl.tools.ui.internal.preferences.SaveParticipantPreferencePage", null, null).open(); //$NON-NLS-1$
            }
          });
...
/*    if (ITextEditorActionConstants.STATUS_CATEGORY_INPUT_POSITION.equals(category)) {
      IStatusField field = getStatusField(IMadlEditorActionConstants.STATUS_CATEGORY_OFFSET);
      if (field != null) {
        ISourceViewer sourceViewer = getSourceViewer();
   ...
   * @param listener The reconcile listener to be added
   */
  final void addReconcileListener_OLD(IMadlReconcilingListener_OLD listener) {
   ...
   * @param listener the reconcile listener to be removed
   */
  final void removeReconcileListener_OLD(IMadlReconcilingListener_OLD listener) {
    synchronized (fReconcilingListeners_OLD) {
