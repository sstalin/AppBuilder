...
public class Util {
  ...
  /**
   * A cached copy of the Madl-like file extensions computed by MadlCore.
   */
  private static char[][] MADL_LIKE_EXTENSIONS;
  ...
  /**
   * Return the registered Madl-like extensions.
   * 
   * @return the registered Madl-like extensions
   * @see MadlCore#getMadlLikeExtensions()
   */
  //ss
/*  public static char[][] getMadlLikeExtensions() {
    if (MADL_LIKE_EXTENSIONS == null) {
      String[] coreExtensions = MadlCore.getMadlLikeExtensions();
      int length = coreExtensions.length;
      char[][] extensions = new char[length][];
      for (int i = 0; i < length; i++) {
        extensions[i] = coreExtensions[i].toCharArray();
      }
      MADL_LIKE_EXTENSIONS = extensions;
    }
    return MADL_LIKE_EXTENSIONS;
  }*/
   ...
   */
  public static String getLineSeparator(String text, MadlProject project) {
  ...
  /**
   * Return the substring of the given file name, ending at the start of a Madl-like extension. The
   * entire file name is returned if it doesn't end with a Madl-like extension.
   * 
   * @return the given file name with any Madl-like extension removed
   */
  //ss
/*  public static String getNameWithoutMadlLikeExtension(String fileName) {
    int index = indexOfMadlLikeExtension(fileName);
    if (index == -1) {
      return fileName;
    }
   ...
   */
  public static char[] getResourceContentsAsCharArray(IFile file) throws MadlModelException {
  ...
  public static char[] getResourceContentsAsCharArray(IFile file, String encoding)
    ...
    if (location == null) {
      ...
      try {
        ...
        if (locationURI == null) {
          ...
          // MadlCore.PLUGIN_ID, Messages.bind(Messages.file_notFound,
          // file.getFullPath().toString())));
          throw new CoreException(new Status(IStatus.ERROR, MadlCore.PLUGIN_ID, "File not found: "
              + file.getFullPath().toString()));
        }
        length = EFS.getStore(locationURI).fetchInfo().getLength();
      } catch (CoreException e) {
        throw new MadlModelException(e, MadlModelStatusConstants.ELEMENT_DOES_NOT_EXIST);
      }
    } else {
      // local file
    ...
    try {
      ...
      throw new MadlModelException(e, MadlModelStatusConstants.IO_EXCEPTION);
    } finally {
      try {
        stream.close();
      } catch (IOException e) {
  ...
  /**
   * Return the index of the Madl-like extension of the given file name or -1 if it doesn't end with
   * a known Madl-like extension. Note this is the index of the '.' even if it is not considered
   * part of the extension.
   * 
   * @return the index of the Madl-like extension of the given file name
   */
  //ss
/*  public static int indexOfMadlLikeExtension(String fileName) {
    int fileNameLength = fileName.length();
    char[][] javaLikeExtensions = getMadlLikeExtensions();
