...
/**
 * Debug/Tracing options for the {@link MadlCore} plugin.
 * 
 * @coverage madl.tools.core
 */
public class MadlCoreDebug {
...
//ss
  ...
  // Verify that madlc has not been specified and that the new analyzer is not explicitly disabled
  public static final boolean ENABLE_NEW_ANALYSIS = true;
  //!isOptionTrue("experimental/analysis/useMadlc")
  //&& !"false".equals(MadlCore.getUserDefinedProperty(ENABLE_NEW_ANALYSIS_USER_FLAG));
  ...
  public static final boolean DISABLE_MADLIUM_DEBUGGER = isOptionTrue("user/disableMadliumDebugger");
  ...
  public static void record(InstrumentationBuilder instrumentation) {
    ...
    instrumentation.metric("DISABLE_MADLIUM_DEBUGGER", DISABLE_MADLIUM_DEBUGGER);
  ...
  private static boolean isOptionValue(String optionSuffix, String expected) {
    String option = MadlCore.PLUGIN_ID + "/" + optionSuffix;
    String value = Platform.getDebugOption(option);
    if (value == null) {
      value = MadlCore.getUserDefinedProperty(option);
    }
