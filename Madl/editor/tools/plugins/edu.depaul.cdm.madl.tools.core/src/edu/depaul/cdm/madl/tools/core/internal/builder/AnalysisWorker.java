...
import edu.depaul.cdm.madl.engine.internal.context.AnalysisOptionsImpl;
...
public class AnalysisWorker {
  ...
  public AnalysisWorker(ContextManager manager, AnalysisContext context) {
    this(manager, context, MadlCore.getProjectManager(), AnalysisMarkerManager.getInstance());
  }
...
/*    MadlSdk sdk;
    synchronized (lock) {
      ...
      sdk = context.getSourceFactory().getMadlSdk();
    }
    boolean hasSdk = sdk != MadlSdkManager.NO_SDK;
    markerManager.queueHasMadlSdk(contextManager.getResource(), hasSdk);
    if (!hasSdk) {
      return;
    }*/
    ...
    while (true) {
      ...
      try {
        ...
        MadlCore.logError("Analysis Failed: " + contextManager, e);
        break;
      } finally {
        builder.log();
      }
    ...
    for (AnalysisListener listener : currentListeners) {
      try {
        ...
        if (!exceptionLogged) {
          ...
          MadlCore.logError("Exception notifying listener that analysis is complete", e);
        }
      }
    }
  }
    ...
    for (AnalysisListener listener : currentListeners) {
      try {
        ...
        if (!exceptionLogged) {
          // Log at most one exception so as not to flood the log
          exceptionLogged = true;
          MadlCore.logError("Exception notifying listener of resolved unit: " + source, e);
        }
      }
    }
  }
    ...
    for (ChangeNotice change : changes) {
      ...
      if (errors != null) {
        if (res == null) {
          ...
          // MadlCore.logError("Failed to determine resource for: " + source);
        } else {
          IPath location = res.getLocation();
          if (location != null && !MadlCore.isContainedInPackages(location.toFile())) {
            LineInfo lineInfo = change.getLineInfo();
            if (lineInfo == null) {
              MadlCore.logError("Missing line information for: " + source);
